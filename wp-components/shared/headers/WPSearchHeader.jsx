...
const WPSearchHeader = () => {
    ...
    if (!loading) {
        if (resultItems && resultItems.items.length > 0) {
            ...
            productItemsView = resultItems.items.filter(searchitem => searchitem.name.toLowerCase().includes(keyword.toLowerCase())).map((product) => (
    ...
    useEffect(()=> {
        if(window !== undefined && isSearch === true){
            const searchPanel = document.querySelector('.ps-panel--search-result')
